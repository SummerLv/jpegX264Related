...
{
	const char* v4l2_dev_name = "/dev/video3";//摄像头设备名
...
static int v4l2_open(char* dev_name)
{
	...
	if (req.count < 2){
     ...
     printf("***    sizeof(*yuyv_buffers0)  :     %d\n",sizeof(*yuyv_buffers0) );
...
static int read_frame(char *filename, int quality)
{
	...
	if(0 != ret){
		...
		printf("exec VIDIOC_DQBUF ****in the read_frame fuction****** success\n");
	}

	//*****  ----  yuvv  weith*height*2  --- YUYV
	// printf("buf.index :\t %d\n", buf.index);
	// 1210
	 yuv_fd = fopen("YUYV422.yuyv","wb");
	 fwrite(yuyv_buffers0[buf.index].start,yuyv_buffers0[buf.index].length,1,yuv_fd);
	 fclose(yuv_fd);
